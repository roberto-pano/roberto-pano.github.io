...
export default function MexicanVisualCultureProject() {
  ...
  return (
    <div>
      <h2 style={{textAlign: 'center', marginBottom: 24}}>Virtual Art Exhibit</h2>
      ...
      <div className="gallery">
        {IMAGES.map((img, idx) => (
          <div className="gallery-item" key={idx}>
            ...
            <img src={img.src} alt={img.alt} className="gallery-img" onClick={() => openModal(img)} style={{cursor:'pointer'}} />
            <div className="description">{img.description}</div>
          </div>
        ))}
      </div>

      <div id="myModal" className={`modal ${modalOpen? 'show' : ''}`} onClick={closeModal}>
        <span className="close" onClick={closeModal}>&times;</span>
        {modalOpen && <img className="modal-content" id="img01" src={modalSrc} alt={modalAlt} />}
